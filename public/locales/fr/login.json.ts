/* eslint-disable */
// For more information, check the common.json.ts file

export default {
  connecting: 'Connexion...',
  'legal.title': 'Création du compte',
  'legal.missingAccount': "Vous n'avez pas encore de compte.",
  'legal.text':
    'En appuyant sur "Créer le compte", vous autorisez l\'utilisation de vos données personnelles comme indiqué dans les <toLegal>Mentions légales</toLegal> du site, et votre compte sera créé.',
  'cgu.button': 'Créer le compte',
  'login.connectWithCas': 'Se connecter avec le CAS',
  'register.registerWithCas': 'créer un compte avec le CAS',
  'login.noAccountYet': 'Pas encore de compte ? Inscrivez-vous !',
  'login.login': 'Se connecter',
  'login.connection': 'CONNEXION',
  'legal.dontConnect': 'Continuer sans compte',
} as const;
